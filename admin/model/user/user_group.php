...
class ModelUserUserGroup extends Model {
    ...
    public function getUserGroups($data = array(), $onewoorkAdmin = false) {
        $addQuery = ($onewoorkAdmin) ? ' WHERE user_group_id !=1' : '';
        $sql = "SELECT * FROM " . DB_PREFIX . "user_group " . $addQuery;
    ...
    public function getTotalUserGroups($onewoorkAdmin = false) {
        $addQuery = ($onewoorkAdmin) ? ' WHERE user_group_id !=1' : '';
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "user_group" . $addQuery);
